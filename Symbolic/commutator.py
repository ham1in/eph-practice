...
H, A, B, C, D, E, F = sp.symbols('H A B C D E F', commutative=False)
...
Hbar2 = H + comm(H, A+ B + C + D + E + F) + c1 * comm(comm(H, A + B + C + D + E + F), A + B + C + D + E + F) + c2 * comm(comm(comm(H, A + B + C + D + E + F), A + B + C + D + E + F), A + B + C + D + E + F) + c3 * comm(comm(comm(comm(H, A + B + C + D + E + F), A + B + C + D + E + F), A + B + C + D + E + F), A + B + C + D + E + F)
...
simp2 = filtered2.subs({A: A_c, B: B_c, C: C_c, D: D_c, E: E_c, F: F_c})
